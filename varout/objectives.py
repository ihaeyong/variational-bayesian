...
def GaussianKL(output_layer):
    ...
    # will get taken apart again in the autodiff
    #return sum([0.5*T.sum(T.log(alpha)) + c1*T.sum(alpha) + c2*T.sum(T.pow(alpha,2)) + c3*T.sum(T.pow(alpha,3)) for alpha in alphas])
    return sum([-0.5*T.sum(T.log((alpha+1)/(alpha))) for alpha in alphas])
def GaussianKLB(output_layer):
    """
    Has the same interface as L2 regularisation in lasagne.
    Input:
        * output_layer - final layer in your network, used to pull the
    ...
    # gather up all the alphas
    params = lasagne.layers.get_all_params(output_layer)
    logalpha = [p for p in params if p.name == "logalpha"]
    w2 = [T.log(T.square(p)) for p in params if p.name == "W"]

    log_alpha = logalpha - w2

    # will get taken apart again in the autodiff
    #return sum([0.5*T.sum(T.log(alpha)) + c1*T.sum(alpha) + c2*T.sum(T.pow(alpha,2)) + c3*T.sum(T.pow(alpha,3)) for alpha in alphas])
    return sum([-0.5*T.sum(T.log1p(T.exp(log_alpha))) for log_alpha in log_alpha])
