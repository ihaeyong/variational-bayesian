...
output_file = "effect_variationalA.pkl.gz"
...
def main(output_dir, verbose=False):
    ...
    for itr in [1, 2, 3, 4, 5]:
        ...
        for n_hidden in [100, 340, 580, 820, 1060, 1300]:
            ...
            l_out = varout.experiments.Effect_vardropADropoutArchitecture(
                    n_hidden=n_hidden)
            # put it in an experiment
            loop = varout.experiments.make_experiment(l_out, dataset,
                    extra_loss=-varout.objectives.GaussianKL(l_out)/50000)
            # run the experiment with early stopping until it converges
            results[n_hidden] = varout.experiments.earlystopping(loop, max_N=200,
                    verbose=verbose)
        # save the results
